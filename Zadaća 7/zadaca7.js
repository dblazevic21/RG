...
function WebGLaplikacija2() 
{
  var platno2 = document.getElementById("canvas2");
  var gl = platno2.getContext("webgl2");
  if (!gl) {
    alert("WebGL2 nije dostupan!");
    return;
  }

  var GPUprogram2 = pripremiGPUprogram(gl, "vertex-shader", "fragment-shader");
  gl.useProgram(GPUprogram2);
  ...
  function napuniSpremnike() 
  {
    GPUprogram2.a_vrhXYZ = gl.getAttribLocation(GPUprogram2, "a_vrhXYZ");
    GPUprogram2.a_boja = gl.getAttribLocation(GPUprogram2, "a_boja");
    GPUprogram2.u_mTrans = gl.getUniformLocation(GPUprogram2, "u_mTrans");
          
    var spremnikVrhova = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, spremnikVrhova);
    gl.enableVertexAttribArray(GPUprogram2.a_vrhXYZ);
    gl.enableVertexAttribArray(GPUprogram2.a_boja);
    gl.vertexAttribPointer(GPUprogram2.a_vrhXYZ, 3, gl.FLOAT, false, 24, 0);
    gl.vertexAttribPointer(GPUprogram2.a_boja, 3, gl.FLOAT, false, 24, 12);
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(vrhovi.flat()), gl.STATIC_DRAW);
  }
  ...
  function iscrtaj() 
  {
    gl.clearColor(0.2, 0.2, 0.2, 1);
    gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);
    gl.viewport(0, 0, platno2.width, platno2.height);
    ...
    gl.uniformMatrix4fv(GPUprogram2.u_mTrans, false, mTrans.lista());
